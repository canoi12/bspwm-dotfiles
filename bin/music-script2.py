...
class PlayerStatus:
    def __init__(self):
        ...
        self._title_color = subprocess.getoutput(
            "xrdb -query ~/.Xresources | grep -w '*.color5' | awk '{print $2}'"
        )
    ...
    def _print_song(self):
        #self._print_flush(
        #    '{}  {} by {}'.format(self._icon, self._artist, self._title))
        color = "%{{F{0}}}".format(self._title_color)
        nc = "%{F-}"
        self._print_flush(
            '{} {} {} by {}'.format(color, self._title, nc, self._artist)
        )
